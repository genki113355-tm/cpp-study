...
3. **C++の文脈へ逆算する**:
   ...
   - **💡 現場の超重要知識（OSによる呼出規約の差分）**:
     Windows環境（Microsoft x64 ABI）では引数は \`RCX, RDX, R8, R9\` の順に渡されますが、Linux や macOS（System V AMD64 ABI）では **\`RDI, RSI, RDX, RCX, R8, R9\`** の順で渡されます。そのため、Linuxゲームサーバーや組込みLinux環境のコアダンプ鑑識では、**「RDI が 0x0 (nullptr) になっていないか」** を真っ先に確認するのがプロの鉄則です！
      `,
      codeFiles: [
        {
