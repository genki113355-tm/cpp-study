...
int main() {
  ...
  quiz: [
    {
      id: 'quiz-m8-1',
      question: 'C++17の if constexpr をテンプレート関数内で使用した際、条件が偽（false）となった分岐のコードはどう扱われますか？',
      options: [
        '実行時にスキップされるだけで、通常通り機械語コードが生成される',
        'コンパイル対象から完全に除外（Discarded Statement）され、その分岐内に含まれる型不適合コードがあっても構文エラーにならず、バイナリも生成されない',
        '条件が偽のブランチがある場合、コンパイラが警告を出力する',
        '実行時に一度だけ評価され、以後はキャッシュされる'
      ],
      correctIndex: 1,
      explanation: 'if constexpr はコンパイル時分岐です。条件が偽となったブランチはコンパイラによって破棄され、テンプレート引数に対して不正なメンバ呼び出し等が含まれていてもエラーにならず、コード生成も一切行われません。'
    },
    {
      id: 'quiz-m8-2',
      question: 'C++17の「構造化束縛（Structured Bindings）」の機能として正しい説明はどれですか？',
      options: [
        '構造体やクラスのメンバ変数をすべて private に強制する機能',
        'auto [x, y, z] = tupleOrStruct; のように、構造体のメンバ、配列、std::pair/std::tuple の要素を1行で分解して各変数に束縛できる機能',
        'クラスの継承関係をコンパイル時に結合する機能',
        '構造体のメモリ配置を1バイト単位にパッキングする機能'
      ],
      correctIndex: 1,
      explanation: '構造化束縛（auto [a, b, ...] = expr;）により、タプルやペア、構造体のメンバを直接別々の変数として受け取ることができるようになり、std::tie や .first/.second などの冗長な記述が不要になりました。'
    },
    {
      id: 'quiz-m8-3',
      question: 'C++17の「フォールド式（Fold Expressions）」の主な目的は何ですか？',
      options: [
        '長い関数をエディタ上で折りたたんで非表示にする機能',
        '可変引数テンプレート（パラメータパック）に対する二項演算（加算や出力など）を、再帰関数を書かずに (... + args) のように1行で展開・集約するため',
        'コンパイル時にコードを難読化・圧縮するため',
        'マルチスレッド処理を自動的にフォールド（合流）させるため'
      ],
      correctIndex: 1,
      explanation: 'フォールド式は可変引数テンプレートの引数リスト（args...）に対する演算（+, *, <<, && など）を、再帰呼び出しテンプレートの定義なしで `(... + args)` のように簡潔に記述できるC++17の新機能です。'
    }
  ]
};
