  ...
  sections: [
    {
      ...
      leadText: '「C言語でも struct を作ってポインタを渡せば同じじゃないか？」という疑問に、オブジェクト指向の核心からお答えします。',
      dialogueBefore: [
        {
          id: 'd2-1',
          speaker: 'penguin',
        ...
        },
        {
          ...
          speaker: 'shirokuma',
          emotion: 'teaching',
          text: 'まさにそこじゃ！その疑問こそ、C言語からC++へ渡る最大の架け橋じゃ。\nC言語の構造体は「データが野ざらし」なんじゃよ。`Player_Move` という正規の関数を用意しても、別の不注意な開発者が `player.x = -9999;` と直接代入するのをC言語の文法では絶対に阻止できん！',
        },
        {
          id: 'd2-3',
          speaker: 'penguin',
          emotion: 'shocked',
          text: 'あ……！確かにC言語の現場でも、「この構造体メンバは直接触るなと関数仕様書に書いたのに、新人が勝手に代入してバグった」って先輩が怒鳴ってました……！',
        },
        {
          id: 'd2-4',
          speaker: 'shirokuma',
          emotion: 'smug',
          text: 'そうじゃ！仕様書（人間の注意）に頼るのがC言語、**【言語仕様（コンパイラ）で物理的に不正代入をブロックする】**のがC++のクラスじゃ！\nデータを private に隠し、public 関数を通さないと触れないようにする。これが『カプセル化（データの隠蔽）』の真意じゃ！',
        }
      ],
      paradigmComparison: {
        title: 'C言語（構造体＋関数） vs C++（クラス＋カプセル化）',
        cApproach: {
          title: 'C言語：データと関数がバラバラ（無防備）',
          code: `typedef struct {
    int x; // 誰でも直接代入できてしまう
    int y;
} Player;
...
// 現場で起きる惨劇：
player.x = -9999; // コンパイルが通ってしまい、画面外へワープ！`,
          drawbacks: [
            '構造体のメンバが全公開（public同等）で改ざんを防げない',
            '「バグが起きた時、全ソースコードから player.x を探す」地獄',
            'データ（構造体）と処理（関数）の結びつきが人間の記憶頼み'
          ]
        },
        cppApproach: {
          title: 'C++：データと操作権の一体化（カプセル化）',
          code: `class Player {
private:
    int m_x; // 外部からはアクセス不可！
    int m_y;
...
// 現場での安全性：
player.m_x = -9999; // コンパイルエラー！ビルド時点でバグを撃滅！`,
          benefits: [
            'privateメンバへの不正代入をコンパイラが100%遮断',
            '「バグの局所化」：自機の位置がおかしければPlayerクラス内だけ見れば解決',
            'オブジェクト指向の神髄「メッセージ送信（player.moveLeft()）」の実現'
          ]
        },
        paradigmShiftNotes: 'C言語は「データを引数で関数に渡して書き換える（外からの操作）」。C++は「オブジェクトが自分のデータを抱え込み、外部からのメッセージに応じて自律的に振る舞う（内からの自己防衛）」。この発想の逆転がカプセル化です。'
      }
    },
    {
      id: 'sec2-file-split',
