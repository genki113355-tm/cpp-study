...
export const chapterL7: Chapter = {
  id: 11,
  slug: 'chapter-classic-7-pointer-alignment-endian',
  courseTrack: 'classic',
...
> [!WARNING]
> **Strict Aliasing Rule（厳格な別名規約）の罠とモダンC++20の解決策**
> \`char*\` や \`uint8_t*\` 以外の異なる型同士でポインタを相互キャストしてアクセス（例: \`*(int*)&myFloat\` や \`reinterpret_cast<int*>(&myFloat)\` による型パンニング）すると、C++規格上の**未定義動作（Undefined Behavior: UB）**となります。コンパイラは「異なる型のポインタは同一メモリ領域を指さない」と仮定して最適化（TBAA: Type-Based Alias Analysis）を行うため、変数の読み書き順序が狂い、Debug版では動くのにReleaseビルドで不可解なバグを生みます。
> - **C++17以前の安全策**: \`std::memcpy(&raw, &val, sizeof(float))\` を使用する（現代の最適化コンパイラは memcpy 呼出を完全に消去し、レジスタ間の単一移動命令に最適化します）。
> - **C++20以降の究極解**: \`<bit>\` ヘッダの \`std::bit_cast<uint32_t>(val)\` を使用する。コンパイル時定数式（\`constexpr\`）でも評価可能で、未定義動作ゼロかつ完全なゼロオーバーヘッドでビット列を安全に再解釈できます。`
    },
    {
...
int main() {
  ...
  prevChapterSlug: 'chapter-classic-6-operator-overload',
  nextChapterSlug: 'chapter-classic-8-function-pointers-callbacks'
};
