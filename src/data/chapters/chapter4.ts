  ...
  sections: [
    {
      ...
      title: '4.1 C言語の「switch(type)地獄」vs C++の「ポリモーフィズム（多態性）」',
      leadText: 'ゲームに新しい敵キャラクターを追加したいとき、C言語プログラマはどう書くでしょうか？ enum で敵種別を定義し、巨大な switch-case 文で分岐させるのが典型です。しかし、その先に待っているのは現場の阿鼻叫喚でした。',
      dialogueBefore: [
        {
          id: 'd4-1',
          speaker: 'penguin',
        ...
        },
        {
          ...
          speaker: 'shirokuma',
          emotion: 'shocked',
          text: '出たな！それこそが全プログラマを破滅に導く【巨大 switch-case の魔窟】じゃ！\n敵の種類が10種類、20種類に増えたとき、お主はすべてのファイルにある数十個の switch 文すべてに `case TYPE_BOSS:` を書き足して回る気か！？ 1箇所でも case を書き忘れたら、画面外へ消えるバグが即座に発生するぞ！',
        },
        {
          id: 'd4-5',
          speaker: 'penguin',
          emotion: 'sweating',
          text: 'ひえぇぇっ！確かに、新しい敵を1つ足すだけで、すでに動いている移動処理も描画処理も当たり判定も全部書き換えなきゃいけなくなります……既存のコードを壊しそうで怖すぎます！',
        },
        {
          id: 'd4-6',
          speaker: 'shirokuma',
          emotion: 'smug',
          text: 'そこで登場するのがオブジェクト指向の最高峰【継承とポリモーフィズム（多態性）】じゃ！\nメインゲームループは『敵よ、動け！』と一言命令するだけ。敵自身が自分が通常敵なのか、シールド敵なのか、UFOなのかを知っており、勝手に最適な動きをする。メインのコードには【1文字も手を触れずに】新しい敵を無限に追加できるのじゃ！',
        }
      ],
      paradigmComparison: {
        title: '多種多様な振る舞いの設計対比：switch(type)分岐 vs ポリモーフィズム',
        cApproach: {
          title: 'C言語：enum ＋ 巨大 switch-case（構造化設計の限界）',
          code: `// C言語での敵種別による分岐
...
void updateEnemy(Enemy* e) {
    switch (e->type) {
        ...
        // プロジェクト中の全 switch 文を修正して回る地獄！
          ...
          drawbacks: [
            '新敵追加のたびに、移動・描画・当たり判定など無数の既存関数を修正する必要がある',
            'case の書き忘れによる未定義動作・サイレントバグの多発',
            '1つの関数に全種類の敵の知識が集約され、神クラス（God function）化して可読性崩壊'
          ]
        },
        cppApproach: {
          title: 'C++：抽象基底クラス ＋ 純粋仮想関数（ポリモーフィズム）',
...
for (size_t i = 0; i < enemies.size(); ++i) {
    enemies[i]->update(); // 実行時に正しい関数の実体が勝手に呼ばれる！
        ...
        paradigmShiftNotes: 'C言語では「呼び出し側（親）が相手の種類を調べて処理を分岐させる」という外向きの制御でした。C++では「呼び出し側はインターフェース（命令）だけを送り、相手（オブジェクト自身）が自分の種類に応じて自律的に振る舞う」という内向きの制御（制御の反転）へとパラダイムシフトします。'
      }
    },
    {
...
void Game::update() {
    ...
    for (size_t i = 0; i < m_enemies.size(); ++i) {
      ...
      takeaways: [
        {
          title: '開放閉鎖原則（OCP）の体現',
          description: '既存のGameクラスのコードに一切手を触れずに、新しい敵クラスを追加するだけで機能拡張が可能。'
        },
        {
          title: '仮想デストラクタの義務化',
          description: '基底クラスポインタ経由でオブジェクトを安全にdeleteするために、基底クラスのデストラクタは必ず virtual にする。'
        },
        {
          title: '動的ディスパッチ（vtable / vptr）',
          description: 'インスタンス内部の隠しポインタ __vptr が、コンパイラが生成した関数テーブルを参照し、実行時に正しい関数へジャンプする。'
        }
      ]
    }
  ],
