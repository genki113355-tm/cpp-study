...
int main() {
  ...
  quiz: [
    {
      id: 'quiz-m6-1',
      question: 'std::string_view が内部で保持しているデータ構造（メンバ）として正しいものはどれですか？',
      options: [
        '独自にヒープ確保した文字列バッファへの所有ポインタとキャパシティ',
        '文字列データの先頭を指すポインタ（const char*）と、その長さ（size_t）の2つのみ',
        'std::string への参照カウンタとミューテックス',
        'null終端文字列のコピー配列（256バイト固定バッファ）'
      ],
      correctIndex: 1,
      explanation: 'std::string_view は文字列の所有権を持たず、既存の文字列の「先頭ポインタ」と「文字数」の2つ（合計16バイト/64bit環境）だけを保持する超軽量ビューです。そのためコピーコストはほぼゼロです。'
    },
    {
      id: 'quiz-m6-2',
      question: '関数の読み取り専用引数に const std::string& ではなく std::string_view を値渡しする最大のメリットは何ですか？',
      options: [
        '関数のコンパイル時間が10倍高速になる',
        '文字列リテラル（"enemy.png"）や const char* を渡した際に、std::string の暗黙の一時オブジェクト生成（ヒープ確保）が一切発生しなくなる',
        '関数内で渡された文字列を自由に書き換えられるようになる',
        'すべての文字列が自動的に暗号化される'
      ],
      correctIndex: 1,
      explanation: 'const std::string& に文字列リテラルを渡すと、暗黙に std::string の一時オブジェクトが作成されヒープ確保（malloc）が走ってしまいます。std::string_view であればポインタと長さをレジスタで直接受け取るため、一切のメモリ確保が起きません。'
    },
    {
      id: 'quiz-m6-3',
      question: 'std::string_view を実戦で利用する際、最も警戒すべき重大な落とし穴はどれですか？',
      options: [
        '日本語（UTF-8）の文字列を渡すと必ず構文エラーになる',
        'std::string_view は所有権を持たないため、参照元の一時オブジェクト破棄後にアクセスするとダングリング参照（未定義動作）になり、末尾に null終端文字（\\0）がある保証もない',
        '長さが10文字以上の文字列を受け取ることができない',
        'マルチスレッド環境で使用すると必ずデッドロックする'
      ],
      correctIndex: 1,
      explanation: 'std::string_view は参照先メモリを所有していないため、一時オブジェクトの寿命が切れるとダングリング参照（未定義動作）になります。また、部分文字列（substr）の切り出し等では末尾に \\0 がないため、printf 等の C言語APIに .data() を直接渡すのは危険です。'
    }
  ]
};
